...
# check files in current director if new files are found, the procede to
# process them
...
if len(current_file_names) >= len(already_ingested_files):
    missing_files = list(
        set(current_file_names).difference(already_ingested_files))
else:
    missing_files = list(
        set(already_ingested_files).difference(current_file_names))
...
logger.info("Full Automation Script Completed...")
